...
public class TestGenericRules extends TestCase {
    ...
    List<Rule> ruleList = Rule.parseRules("[r1: (?a p ?b), (?b p ?c) -> (?a p ?c)]" +
            "[r2: (?a q ?b) -> (?a p ?c)]" +
            "-> table(p). -> table(q).");
    Triple[] ans = new Triple[]{new Triple(a, p, b), new Triple(b, p, c), new Triple(a, p, c)};
...
//        return suite;
    ...
    public void testForward() {
        ...
        test.add(new Triple(a, p, b));          // todo: use from our database
    ...
    public void testHybrid() {
        ...
        List<Rule> rules = Rule.parseRules("[a1: -> (a rdf:type t)]" +
                        "[r0: (?x r ?y) -> (?x p ?y)]" +
        ...
        TestUtil.assertIteratorValues(this, infgraph.find(null, p, null), new Object[]{new Triple(a, p, a), new
                Triple(a, p, b), new Triple(b, p, a)});
...
//        out.close();
    ...
    public void testBRuleErrorHandling() {
        Graph data = Factory.createGraphMem();
        List<Rule> rules = Rule.parseRules("[a1: -> [(?x eg:p ?y) (?x eg:q ?y) <- (?x eg:r ?y)]]");
    ...
    public void testParameters() {
        ...
        configuration.addProperty(ReasonerVocabulary.PROPruleSet, "D:\\projects\\lpt\\entice-ul-api\\src\\main" +
                "\\webapp\\WEB-INF\\classes\\genericRuleTest.rules");
        GenericRuleReasoner reasoner = (GenericRuleReasoner) GenericRuleReasonerFactory.theInstance().create
                (configuration);
        ...
        TestUtil.assertIteratorValues(this, newConfig.listProperties(), new Statement[]{m2.createStatement(newConfig,
                ReasonerVocabulary.PROPderivationLogging, "true"), m2.createStatement(newConfig, ReasonerVocabulary
                .PROPruleMode, "hybrid"), m2.createStatement(newConfig, ReasonerVocabulary.PROPruleSet,
                "testing/reasoners/genericRuleTest.rules")});
        ...
        reasoner.addDescription(m2, newConfig);
        TestUtil.assertIteratorValues(this, newConfig.listProperties(), new Statement[]{m2.createStatement(newConfig,
                ReasonerVocabulary.PROPderivationLogging, "false"), m2.createStatement(newConfig, ReasonerVocabulary
                .PROPruleMode, "hybrid"), m2.createStatement(newConfig, ReasonerVocabulary.PROPruleSet,
                "testing/reasoners/genericRuleTest.rules")});
        ...
        TestUtil.assertIteratorValues(this, infgraph.find(null, null, null), new Object[]{new Triple(an, RDF.Nodes
                .type, C), new Triple(an, RDF.Nodes.type, D),});
    ...
    public void testHybridFunctorFilter() {
        ...
        InfGraph infgraph = reasoner.bind(data);
        TestUtil.assertIteratorValues(this, infgraph.find(null, q, null), new Object[]{});

        reasoner.setFunctorFiltering(false);
        infgraph = reasoner.bind(data);
        TestUtil.assertIteratorValues(this, infgraph.find(null, q, null), new Object[]{new Triple(a, q, Functor
                .makeFunctorNode("func", new Node[]{b, s}))});
    }
    ...
    public void testIncludeRedirect() {
        ...
        assertTrue(checkIncludeFound("D:\\projects\\lpt\\entice-ul-api\\src\\main\\webapp\\WEB-INF\\classes" +
                "\\importTest.rules"));
    ...
    public void doTestAddRemove(boolean useTGC) {
        ...
        List<Rule> rules = Rule.parseRules("-> table(rdf:type)." +
                        "[r1: (?x p ?c) -> (?x rdf:type ?c)] " +
                        "[rdfs9:  (?x rdfs:subClassOf ?y) -> [ (?a rdf:type ?y) <- (?a rdf:type ?x)] ]");
        if (!useTGC) {
            rules.add(Rule.parseRule("[rdfs8:  (?a rdfs:subClassOf ?b), (?b rdfs:subClassOf ?c) -> (?a " +
                    "rdfs:subClassOf ?c)] "));
        }
        GenericRuleReasoner reasoner = (GenericRuleReasoner) GenericRuleReasonerFactory.theInstance().create(null);
        reasoner.setRules(rules);
//        reasoner.setTraceOn(true);
        ...
        InfGraph infgraph = reasoner.bind(data);
        TestUtil.assertIteratorValues(this, infgraph.find(a, ty, null), new Object[]{new Triple(a, ty, C1), new
                Triple(a, ty, C2), new Triple(a, ty, C3)});
        ...
        TestUtil.assertIteratorValues(this, infgraph.find(a, ty, null), new Object[]{new Triple(a, ty, C1), new
                Triple(a, ty, C3)});
        TestUtil.assertIteratorValues(this, infgraph.find(b, ty, null), new Object[]{new Triple(b, ty, C2), new
                Triple(b, ty, C3)});

        TestUtil.assertIteratorValues(this, data.find(null, null, null), new Object[]{new Triple(a, p, C1), new
                Triple(b, p, C2), new Triple(C2, sC, C3), new Triple(C1, sC, C3)});
    }
    ...
    public void testAddRemove2() {
        ...
        List<Rule> rules = Rule.parseRules("(?x p ?v)-> (?x q inc(1, a)).\n" +
                        "(?x p ?v)-> (?x q inc(1, b)).\n" +
                        "(?x p ?v) (?x q inc(?i, ?t)) noValue(?x r ?t) sum(?v, ?i, ?s) -> remove(0,1), (?x p ?s) (?x " +
                "r ?t).\n");
...
//        "(?x p ?v) (?x q inc(?i, ?t)) sum(?v, ?i, ?s) -> remove(0,1), (?x p ?s).\n");
        ...
        InfGraph infgraph = reasoner.bind(data);
        TestUtil.assertIteratorValues(this, infgraph.find(a, p, null), new Object[]{new Triple(a, p, Util.makeIntNode
                (2))});
    }
}